...
def get_category(category_name):
    ...
    """
    result_category = session.query(Category).filter_by(name=category_name).one()
...
def update_category(category_name):
    ...
    """
    name = request.form['name']
    category_to_update = session.query(Category).filter_by(name=category_name).one()
...
def delete_category(category_name):
    ...
    target_category = session.query(Category).filter_by(name=category_name).one()
    items_in_category_to_delete = session.query(Item).filter_by(category_id=target_category.id).all()

    for i in items_in_category_to_delete:
        delete_item(i.name)

    category_to_delete = session.query(Category).filter_by(name=category_name).delete()
    session.commit()
